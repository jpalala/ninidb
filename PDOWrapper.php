...
class PDOWrapper {
	...
	/**
	 * method execute.
	 * 	- executes a query that modifies  (update/delete/truncate/create) the database
	 *
	 * @param string $query - the SQL query we are executing
	 * @param array $params - a list of bind parameters
	 * @param bool $use_master - use the master or not
	 * @return mixed - the affected rows, false on failure
	 */
	public function execute($query, $params=array(),$use_master = false) {
		try {
			// use the connection specified by $use_master
			$pdo_connection = $use_master ? $this->getMaster() : $this->getSlave();
